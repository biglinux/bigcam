...
set -Eeuo pipefail
[[ $EUID != 0 ]] || {
	echo "Run graphical tests as a normal user" >&2
	exit 1
}
...
export HOME="$XDG_RUNTIME_DIR/home" XDG_CONFIG_HOME="$XDG_RUNTIME_DIR/config" XDG_CACHE_HOME="$XDG_RUNTIME_DIR/cache" XDG_STATE_HOME="$XDG_RUNTIME_DIR/state" XDG_DATA_HOME="$XDG_RUNTIME_DIR/data"
mkdir -p "$HOME" "$XDG_CONFIG_HOME" "$XDG_CACHE_HOME" "$XDG_STATE_HOME" "$XDG_DATA_HOME"
export GTK_A11Y=atspi NO_AT_BRIDGE=0 GDK_BACKEND=x11 GSK_RENDERER=cairo LIBGL_ALWAYS_SOFTWARE=1
...
trap cleanup EXIT
xvfb-run -a -s '-screen 0 1280x900x24 -nolisten tcp' dbus-run-session -- bash -s -- "$ROOT" <<'SESSION'
    set -euo pipefail
    ...
    /usr/bin/python3 -m pytest -q -p no:cacheprovider "$1/tests/integration/test_recording.py"
