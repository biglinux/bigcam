...
class StreamEngine(GObject.Object):
    ...
    def _build_appsink_pipeline(self, gst_source: str) -> bool:
        ...
        self._appsink_max_retries = 20  # 20 * 500ms = 10s max wait
    ...
    def _try_appsink_pipeline(self) -> bool:
        ...
        for i, pipeline_str in enumerate(pipeline_attempts):
            log.info("Pipeline attempt %d-%d (appsink): %s",
                     self._appsink_retry_count, i + 1, pipeline_str)
            try:
    ...
    def _on_appsink_sample(self, appsink: Any) -> Gst.FlowReturn:
        ...
        if result:
            ...
            buf.unmap(map_info)
            GLib.idle_add(self._update_texture, w, h, glib_bytes)
        return Gst.FlowReturn.OK

    def _update_texture(self, w: int, h: int, glib_bytes: GLib.Bytes) -> bool:
        try:
            texture = Gdk.MemoryTexture.new(
                w, h, Gdk.MemoryFormat.R8G8B8, glib_bytes, w * 3
            )
